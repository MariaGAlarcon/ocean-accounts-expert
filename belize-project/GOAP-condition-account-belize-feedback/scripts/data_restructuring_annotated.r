# =============================================================================
# data_restructuring_annotated.r
# =============================================================================
# PURPOSE: Reads 4 AGRRA Excel export files (BenthicRaw, CoralRaw, FishRaw,
# Metadata) and restructures them into standardized dataframes for validation
# and indicator calculation.
#
# ORIGINAL SOURCE: GOAP-condition-account-belize repository
#
# OVERVIEW OF CHANGES NEEDED:
# - Lines 43, 58, 79, 98, 118, 140 each contain a hardcoded subregion filter
#   that restricts data to "Northern Barrier Complex" only, discarding all
#   other subregions. For national-level accounts, this filter must be removed
#   or made configurable.
# - Year extraction is derived from survey dates. The AGRRA metadata includes
#   a Batch table (sheet 2) with an explicit Year field per batch, which is
#   more reliable for mapping surveys to accounting periods.
#
# See feedback_report.md for full discussion.
# =============================================================================
...
# [FEEDBACK] HARDCODED SUBREGION FILTER (1 of 6)
# The filter below restricts df_sites to "Northern Barrier Complex" only.
# This discards all sites from other subregions (Central Barrier Complex,
# Southern Barrier Complex, Glover's Reef Atoll, Turneffe Atoll, Lighthouse
# Reef). To process all subregions, remove this filter or replace it with a
# configurable parameter at the top of the script, e.g.:
#   target_subregion <- "Northern Barrier Complex"  # or NULL for all
#   ... %>% { if (!is.null(target_subregion)) filter(., Subregion == target_subregion) else . }
df_sites <- df_surveys %>%
...
# [FEEDBACK] HARDCODED SUBREGION FILTER (2 of 6)
# Same issue as above. Filters benthic cover to Northern Barrier Complex only.
...
# [FEEDBACK] HARDCODED SUBREGION FILTER (3 of 6)
# Same issue. Filters recruits to Northern Barrier Complex only.
...
# [FEEDBACK] HARDCODED SUBREGION FILTER (4 of 6)
# Same issue. Filters invertebrates to Northern Barrier Complex only.
...
# [FEEDBACK] HARDCODED SUBREGION FILTER (5 of 6)
# Same issue. Filters coral community to Northern Barrier Complex only.
...
# [FEEDBACK] HARDCODED SUBREGION FILTER (6 of 6)
# Same issue. Filters fish data to Northern Barrier Complex only.
