# =============================================================================
# data_validation_annotated.r
# =============================================================================
# PURPOSE: Validates each restructured dataframe against expected schemas,
# checking column completeness, value ranges, enumerated values, and
# date/time formats. Writes a validation report to outputs/.
#
# ORIGINAL SOURCE: GOAP-condition-account-belize repository
#
# OVERVIEW OF CHANGES NEEDED:
# - Line 43: values_site_codes is hardcoded to 11 specific site codes from the
#   original pilot. Real AGRRA data contains 169 sites across 6 subregions.
#   These codes should be derived dynamically from the data.
# - Line 44: values_mpa_names is hardcoded but never used in the restructured
#   data (MPA_Management is set to NA in data_restructuring.r). This check
#   always passes vacuously. Either populate MPA assignments upstream or
#   remove this validation.
# - Several checks are too rigid for new AGRRA campaigns where site codes,
#   MPA boundaries, and subregion names may differ from the original pilot.
#
...
# [FEEDBACK] HARDCODED SITE CODES
# These 11 codes are specific to the Northern Barrier Complex pilot. The full
# AGRRA dataset contains 169 sites across 6 subregions with different codes.
# Any new site not in this list triggers a validation failure. To make this
# dynamic, derive site codes from the data itself:
#   values_site_codes <- unique(df_sites$Site)
# This still validates that all downstream dataframes reference sites that
# exist in the sites table, without rejecting legitimate new sites.
values_site_codes <- c("MXRCK", "BZ1077", "BZ1079", "BZ1081", "BZ1230", "BZ1231", "BZHCCD01", "BZHCCD02", "CCSC", "BZ1229", "1076B")

# [FEEDBACK] HARDCODED MPA NAMES
# This list is validated against df_sites$MPA_Management, but that column is
# set to NA in data_restructuring.r. The check passes vacuously because all
# values are NA. If MPA assignments are added upstream, this list would need
# to cover all Belize MPAs. Deriving dynamically would be safer:
#   values_mpa_names <- unique(na.omit(df_sites$MPA_Management))
